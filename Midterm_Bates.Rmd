...
```
>When we're assessing the anomalous significance of `r p3.event` people dying in a population, we are going to assess $$Pr(X>=`r p3.event`|X~binom(`r p3.n`, `r p3.p`)) or the right tailed test

>And because $$pbinom(x, ... , lower.tail = FALSE)$$ computes values for values strictly greater than x, we will have to subtract 1 from our event number `r p3.event`.

```{r}
pbinom(p3.event - 1, size = p3.n, prob = p3.p, lower.tail = FALSE)

```
> All semester, we have been utilizing $$p < 0.05$$ as an indicator for statistical significance, I would say that a value of $$`r pbinom(p3.event - 1, size = p3.n, prob = p3.p, lower.tail = FALSE)`$$ not sufficient to warent any concern.
...
```{r}
p8.ls$coefficients[1] + 0.5*p8.ls$coefficients[2]
...
cor.test(p9.x, p9.y, alternative = 'two.sided')
...
p10.ls <- lm(p10.bp ~ p10.bw + p10.age)
summary(p10.ls)
...
```{r}
p10.ls$coefficients
data.frame(bw=c(8), age=(3))

predict(p10.ls,data.frame(bw=c(8), age=(3)))
