...
>The value of c that contains 98% of all values is `r p2.c`
...
>Since the F test for equal variances shows us that we cannont reject the null hypothesis of equal variances, becuase `r p5.var$p.value` > 0.05, we will utilize the t.test with var=TRUE.


```{r}

p5.t.alt <- t.test(pipe, brush, var.equal = TRUE)
p5.t.alt

```
> When we look at the t test with equal variances, we see that the p value is `r p5.t.alt$p.value`, which is > 0.5.  So we would fail to reject the null of equal means.
...
p6.allergics.st <- shapiro.test(p6.allergics)
p6.non_allergics.st <- shapiro.test(p6.nonallergics)

p6.allergics.st
p6.non_allergics.st
```

>We can see that for the Allergics, the Shapiro-Wilk Normality test returns a p value of `r p6.allergics.st$p.value`, which is < 0.05.  We therefore can reject the null hypothesis that the data is normally distributed.

>For the Non-Allergics, we see that the p value is `r p6.non_allergics.st$p.value`, which is > 0.05, which means that we cannot reject the null hypothesis of normally distributed data.
...
> With a p value of `r p6.w$p.val`, we would reject the null hypothesis even if alpha is 0.01
...
p8.predict <- predict(p8.ls, newdata = data.frame(NO=c(0.5)))

unname(p8.predict)
```
>The predicted Lichens N for a NO value of 0.5 is `r unname(p8.predict)`
...
> The multiple linear regression model equation is bp = `r p10.ls$coefficients[1]` + `r p10.ls$coefficients[2]`(bw) + `r p10.ls$coefficients[3]` (age)

```{r,fig.align='center', fig.height=10, fig.width=10, include=FALSE}
par(mfrow=c(2,2))
plot(p10.ls)
...
p10.predict <- predict(p10.ls, newdata = data.frame(bw=c(p10.oz), age=c(3)))


```
>The predeicted bp for the baby is `r unname(p10.predict)`
...
> For every 1 oz in birth weight, while holding everything else constant, we can expect the BP to increase `r p10.ls$coefficients[2]`
> For every day in age, while holding everything else constant, we can expect the the BB to increase `r p10.ls$coefficients[3]`
...
legend("bottomright", col =  c("red", "green", "blue"), fill = unique(iris$Species), legend = c( levels(iris$Species)),)
...
>There appear to be 150 different observations with 50 different observations between 3 different species. Sepal.Width and Petal.Width seem to have the least variance between the maximum and minimul values.  THe petal.length appears to have the greatest variation.
