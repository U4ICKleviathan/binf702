...
```{r, label='impute-means-mrna', eval=TRUE, include=FALSE}
...
Following this step, I then removed samples where all of the values for the gene expressions were 0. Additionally, I removed all the patient data for those who’s status of N0/N1 was not known. At the end of this step, we had 420 patient records with a total of `r dim(mrna)[1]` gene expression features. This resulted in the following distribution of N0/N1 across the patients:
...
I then took the ANOVA reduced mrna samples and processed them throug the Naive Bayes Classifier to investigate other potential features of importance.  When the model was completed, I found the following variables of importance:
...
```{r, label='kmeans-mrna-SMALL-1', eval=TRUE, include=TRUE, fig.height=8, fig.width=8, fig.cap="Fig 4: K-Means Clustering of SPAG1 and HIST1H3J"}
...
```{r, label='kmeans-mrna-SMALL-2', eval=TRUE, include=TRUE, fig.height=8, fig.width=8, fig.cap="Fig 5: K-Means Clustering of SPAG1 and RALGPS1"}
...
```{r, label='kmeans-mrna-SMALL-3', eval=TRUE, include=TRUE, fig.height=8, fig.width=8, fig.cap="Fig 6: K-Means Clustering of HIST1H3J and RALGPS1"}
...
## Discussion
 ...
 In reviewing the literature for SPAG1, RALGPS1, and HIST1H3J I discovered that these have been implicated in poor outcomes with respect to other cancers.  SPAG1 is sperm associated antigen 1, and has been reported as an overexpressed gene in squamous cell lung carcinoma with metastatic properties @boelensGenomicAberrationsSquamous2009. There is another article which shows that the expression of this gene correlates with metastatic features and tumor genesis of pancreatic tumors @neesseSpermassociatedAntigenExpressed2007.  SPAG1 has also been identified in other literature to have elevated expression in prostate cancers, particularly with those of a high Gleason Score (> 8) @jhunGeneExpressionSignature2017.
 
 RALGPS1 has also been linked with negative cancer outcomes in other papers.  It does this by activating Ral small GTPase, and Ral small GTPase activation has been reported in human cancers as well as being implicated in highly aggressive cancers @smithTranscriptionalSignaturesRal2012.
 
 
 HIST1H3J is an interesting feature in that it is a histone coding protein.  There were a few articles which discussed HIST1H3J in it's involvement in cancers. One article did not report to have any causal benefits to cancers, but rather correlation with worse overall survival of cancers @xiangNetworkPharmacologyBioinformatics2021.
 
## Conclusion 
 
 Through various machine learning methods, I was able to identify a few target genes. Both the Random forest and Naive Bayes classifier turned up similar genes, but each of these methods uncovered different 'most important' features.  And when we plotted the various important features as a ROC Curve, we saw that not every feature that was listed as important displayed a significant area under the curve.
 
While it is encouraging to find other references in the literature which implicate the genes we discovered as indicators for negative cancer outcomes, I still believe that I have more work do to progress in creating a model for identifying genes that would indicate a spread to the lymph nodes. As we saw, the confusion matrix for both the Random Forest as well as the Naive Bayes did not have a high accuracy and did not perform very well against the test data set.  These initial markers I identified are a start, but I would propose further tests to reduce the features even further.  The Wilcox test would possibly be the next step in building a subset of features that would possibly increase the predictive accuracy of the classifiers.
