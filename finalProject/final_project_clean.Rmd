...
```{r label='install-packages', eval=TRUE}
...
```{r, label='read-curated-data', eval=FALSE}

ref_data <- read.csv('CopyOfdata_curated_trans.csv')

```

```{r, label='identify-records', eval=FALSE}
test <- grep("SSX9", names(ref_data), value = TRUE)
test <- grep("NSMCE1", names(ref_data), value = TRUE)
idx <- match(test, names(ref_data))

patinet_index = ref_data[ref_data['PATIENT_ID']=='TCGA-2A-A8VL',]
patinet_index[,idx]



for(name in names(study.all_data)){
  td <- getWithColData(study.all_data, name)
  df <- assay(td)
  d <- as.data.frame(t(df))
  print(paste(name,": ", d['TCGA-2A-A8VL','SSX9']))
...
```{r, label='extract-both-experiment-data', eval=FALSE}
rel_experiments <-
  subsetByAssay(study.all_data,
                 c('mrna_seq_v2_rsem_zscores_ref_diploid_samples', 'cna'))
names(rel_experiments) <- c('mrna', 'cna')
rel_experiments

experiment_df <- wideFormat(
  rel_experiments,
  colDataCols = c(
    "PATIENT_ID",
    "PATH_N_STAGE",
...
```{r, label='difference-between-published-data', eval=FALSE}

ref_names <- names(ref_data)
exp_names <- names(experiment_df)

test_vec <- c('a', 'b','c')

exp_upper <- unname(sapply(exp_names, toupper))
ref_upper <- unname(sapply(ref_names, toupper))

diff_names <- unname(setdiff.Vector(ref_names, exp_names))
diff_upper <- unname(setdiff.Vector(ref_upper, exp_upper))

in_exp <- intersect.Vector(diff_names, exp_names)
length(in_exp)

in_exp_upper <- intersect.Vector(diff_upper, exp_upper)
length(in_exp_upper)

in_ref <- intersect.Vector(diff_names, ref_names)
in_ref[1:3]
ref_data[1,in_ref[1:3]]
length(in_ref)

in_ref_upper <- intersect.Vector(diff_upper, ref_upper)
length(in_ref_upper)
in_ref_upper[1:3]

grep('*CXORF67',ref_names, ignore.case =TRUE, value = TRUE)
grep('*CXORF67*',exp_names, ignore.case =TRUE, value = TRUE)



clean_diff <- unname(sapply(in_ref,function(x) gsub('^M_', '', x)))
clean_diff[1:3]
clean_in_exp <- intersect.Vector(exp_names, clean_diff)
length(clean_in_exp)
grep('^CX', exp_names, value = TRUE)
...
```{r, label="get-cna-data", eval=FALSE}
cna_subset <- subsetByAssay(study.all_data, 'cna')
names(cna_subset) <- c("cna")
...
cna.data[1,1:3]
cna.df <- as.data.frame(cna.data)
cna.df[1,patientCols]
cna.df <- cna.df[,c(patientCols, setdiff(names(cna.df), patientCols))]
cna.df <- cna.df[,!names(cna.df) %in% c('primary')]
cna.df[1,1:10]
cna.df.non_num_cols <- patientCols
cna.df.num_cols <- unname(setdiff(names(cna.df), patientCols))

cols_to_keep <- colSums(cna.df[,cna.df.num_cols])!=0
cna.df <- cna.df[, c(cna.df.non_num_cols, cna.df.num_cols[cols_to_keep])]

dim(cna.data)[2]-length(patientCols)
dim(cna.df)[2]-length(patientCols)
...
```{r, label="get-mrna-data", eval=TRUE}
...
```{r, label='data-cleanup-remove-cols-with-0-variance', eval=FALSE}
# https://rpubs.com/Mentors_Ubiqum/Zero_Variance

ref_names <- names(ref_data)
ref_mrna <- startsWith(ref_names, 'M_')
ref_m_names <- ref_names[ref_mrna]
ref_m_names[1]
table(ref_mrna)
ref_test <- ref_data[ref_m_names]
dim(ref_test)
ref_test <- ref_test[, colSums(is.na(ref_test)) != nrow(ref_test)]
ref_test

for (i in 1:ncol(ref_test)) {
  ref_test[, i][is.na(ref_test[, i])] <-
    mean(ref_test[, i], na.rm = TRUE)
}

which(apply(ref_test, 2, var) == 0)

test <- apply(mrna.df[, mrna.df.num_cols], 2, var)
test_o <- order(test, decreasing = TRUE)
test[test_o[1]]

nonvar_cols <- which(apply(test[mrna.df.num_cols], 2, var) != 0)
length(nonvar_cols)
...
```{r, label='cor-variables', eval=T}
# https://stackoverflow.com/questions/19113181/removing-na-in-correlation-matrix
library(caret)

mrna.cor <-cor(t(exprs(mrna.train)))
mrna.cor[1:3,1:3]

drop <- findCorrelation(mrna.cor , cutoff = 0.9)
r <- mrna.train[-drop, ]
dims(r)
rm(mrna.cor)

cormrna <- apply(exprs(r), 1, function(x) cor(x, r$PATH_N_STAGE_FAC))
...
par(mfrow = c(2, 2))
signif <- c("mrna_SPAG1", "mrna_PLEKHF2", "mrna_RALGPS1", "mrna_OMP", "mrna_CTNNAL1", "mrna_ZBTB34", "mrna_ODF2", "mrna_FKBP15", "mrna_GOLGA1")

for(i in c(1:4)){
pred <- prediction(t(exprs(mrna.train[signif[i]])), mrna.train$PATH_N_STAGE)
perf <- performance(pred, "tpr", "fpr" )
plot(perf,
     lwd=4,
     col="magenta",
...
```{r, label='PCAtools-test-mrna', eval=FALSE}
library(PCAtools)
# https://bioconductor.org/packages/release/bioc/vignettes/PCAtools/inst/doc/PCAtools.html

t <- mrna.train@phenoData
t1 <- as(t, 'data.frame')

p <- pca(t(exprs(mrna.train)), metadata=t1, removeVar = 0.1)

var <-getVars(p)



screeplot(p, axisLabSize = 18, titleLabSize = 22, components = getComponents(p, 1:15))
...
```{r, label='princomp-mrna-like-golub'}

pca <- princomp(exprs(mrna.train), center=TRUE, scale = TRUE)

o <- order(pca$scores[,3])
top_10_names <- featureNames(mrna.train)[o[1:10]]

par(mfrow = c(2, 2))

for(i in c(1:4)){
pred <- prediction(t(exprs(mrna.train[top_10_names[1],])), mrna.train$PATH_N_STAGE)
perf <- performance(pred, "tpr", "fpr" )
plot(perf,
     lwd=4,
     col="magenta",
...
gene<-'mrna_BRAP'
df <- data.frame(gene=exprs(mrna)[gene,], PATH_N=mrna$PATH_N_STAGE)


ggplot(df, aes(x=gene, fill=PATH_N))+ geom_histogram()+ggtitle(gene)

gene<-'mrna_SPAG1'
df <- data.frame(gene=exprs(mrna)[gene,], PATH_N=mrna$PATH_N_STAGE)

ggplot(df, aes(x=gene, fill=PATH_N))+ geom_histogram(binwidth=.5 ,alpha=.5, position="identity")+ggtitle(gene)
...
min(cor(exprs(mrnaSp)))
eigen(cor(exprs(mrnaSp)))$values[1:5]

data <- exprs(mrnaSp); p <- ncol(mrnaSp); n <- nrow(mrnaSp) ; nboot<-1000
...
pca <- princomp(exprs(mrnaSp), center = TRUE, cor=TRUE, scores=TRUE)

summary(pca)
ggbiplot(pca)

o <- order(pca$scores[,2])
on <-  rownames(mrnaSp)[o]
grep('mrna_SPAG1',on)
?grep
'mrna_SPAG1' %in% rownames(mrnaSp)
...
data <- data.frame(t(exprs(mrna.train[fi[1]])))

n_factor <- factor(mrna.train$PATH_N_STAGE, labels= c("N0","N1"))
t<-t(exprs(mrna.train[fi[1]]))

stripchart(t~n_factor, pch=as.numeric(n_factor), method="jitter")

cl <- kmeans(data, centers=2)
table(cl$cluster,n_factor)
...
plot(
  t,
  pch = as.numeric(mrna.train$PATH_N_STAGE_FAC) + 14,
  col = as.numeric(mrna.train$PATH_N_STAGE_FAC) + 1,
  xlab = 'Patient Index'
)



cl <- kmeans(t(exprs(mrna.train[forrest_importance[1],])), 2)



plot(t, 
     col = cl$cluster,
     pch = as.numeric(mrna.train$PATH_N_STAGE_FAC) + 15,
     xlab = 'patient index'
     )
...
sd.data=scale(exprs(mrna.train))
...
table(mrna.train$PATH_N_STAGE, km.out$cluster)

plot(mrna.train.features[,t_c], col = cl$cluster)
points(cl$centers, col = 1:2, pch = 8, cex=2)

grep('RALGPS1', feat_names, value = TRUE)
...
```{r, label='kmeans-mrna-SMALL'}
feat_names <- names(train_features)[!(names(train_features) %in% patientCols)]
...
```{r, label='from-notebook'}
signif <- c("mrna_SPAG1", "mrna_PLEKHF2", "mrna_RALGPS1", "mrna_OMP", "mrna_CTNNAL1", "mrna_ZBTB34", "mrna_ODF2", "mrna_FKBP15", "mrna_GOLGA1")

train_features[1, c("mrna_SPAG1", "mrna_PLEKHF2", "mrna_RALGPS1", "mrna_OMP", "mrna_CTNNAL1", "mrna_ZBTB34", "mrna_ODF2", "mrna_FKBP15", "mrna_GOLGA1")]
